...
def check_anwer(guesses):
    """
    Check if the user's input matches the answer

    Inputs:
        index: from play_game(), indicates which blank we are checking
        level: difficulty level parsed from play_game()
        gusss_num: max number of guesses allowed parsed from play_game()

    Outputs:
        just two print statements depends on the user's input
    """
    guesses += 1

def play_game(level, guess_num):
    """
    ...
    Outputs:
        A congratulation phrase if the user passes this game. If not, repeat
        the game until success
    """
        ...
        if "____" in word:
            ...
            while guesses < guess_num:
                ...
                if response != quiz[level]['answers'][index]:
                    ...
                    print "You're correct! The " + index_map[index] + \
                          " blank should be " + quiz[level]['answers'][index]
            ...
            if guesses == guess_num:
                print "Sorry, you guessed " + str(guess_num) + \
                      " times wrong. You lost this game! Please restart " \
                      "this game."
                restart_game = True
                break
